<template>
  <div class="q-pa-md">
    ...
    <q-dialog v-model="showDetail" persistent>
      <q-card style="min-width: 400px; max-width: 800px" class="order-detail-card">
        <!-- Header -->
        ...
        <q-card-section class="q-pa-none">
          ...
          <div
            v-if="getOrderItems(detail).length > 0"
            class="q-px-md"
          >
            <q-list separator>
              <q-item
          ...
          <div v-else class="q-pa-md text-center text-grey-6">
            ...
            <div class="text-body2 q-mt-sm">No items available for this order</div>
          </div>
        </q-card-section>
        ...
        <q-card-section>
          ...
          <div class="order-summary">
            <div class="row justify-between q-mb-sm">
              ...
              <span class="text-body2">Rs {{ formatPrice(detail.subtotal ?? detail.total ?? 0) }}</span>
            </div>
            <div class="row justify-between q-mb-sm">
              <span class="text-body2">Shipping</span>
              <span class="text-body2">Rs {{ formatPrice(detail.shipping ?? 0) }}</span>
            </div>
            <div class="row justify-between q-mb-md">
              <span class="text-body2">Tax</span>
              <span class="text-body2">Rs {{ formatPrice(detail.tax ?? 0) }}</span>
            </div>
        ...
        <q-card-actions align="right" class="q-pa-md">
          ...
          <q-btn
            ...
            no-caps
            icon="track_changes"
            v-if="detail.status !== 'delivered' && detail.status !== 'cancelled' && detail.status !== 'refunded'"
          />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
const viewOrder = async (order) => {
  if (order.items || order.order_items) {
    detail.value = {
      ...order,
      items: order.items || (order.order_items ? order.order_items.map((item) => ({
